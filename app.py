...
def register():    
    if request.method =="POST":     
        ...
        # In your registration route, validate password:
        if not is_strong_password(password):
            ...
            # flash("Flask Validation Sample", "error")
...
def tasks():
    ...
    if session["role"] == "user":
        c.execute("SELECT * FROM tasks WHERE created_by=?",
                  (session["user_id"],))
